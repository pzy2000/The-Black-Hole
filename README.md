...
把黑洞挂上桌面或屏保：**「朝向黑洞」机位 + 时间 ×10 + 飞船匀速圆周轨道**的影院模式
（`?cinema=1`），无 UI、无输入、永不坠落，一整圈约 64 秒无限循环；默认「超高」画质、
不限帧率，选「自动」画质时帧率不足会逐级降档（超高 → 高 → 中）。

```bash
./macos/build.sh     # 构建网页 dist + 两个宿主（Swift 编译，无需 Xcode 工程）
./macos/install.sh   # 壁纸 App → /Applications；屏保 → ~/Library/Screen Savers
python3 macos/e2e.py # 设置功能端到端测试（37 项，需 GUI 会话）
```
...
`~/Library/Application Support/EventHorizon/settings.json`，改动即时生效）：**

- **性能**：画质（超高默认 / 高 / 中 / 低 / 自动降级）、最大帧率（不限默认 / 120 / 60 / 30）。
- **失焦时**（被全屏应用遮挡 / 屏保开始，逐屏独立生效）：
  暂停渲染（默认，零 GPU 占用）/ 停止并释放内存 / 静音 / 继续运行。
- **背景音乐**：自动扫描 `assets/bgms/`（构建时打包进两个宿主），可选曲目起点、
  列表循环 / 随机 / 单曲循环、音量；多屏只有主屏一路出声。
- 影院模式也可直接在浏览器打开：`npm run dev` 后访问 `http://localhost:5188/?cinema=1`。
